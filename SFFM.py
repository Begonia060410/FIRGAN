...
class SFFM(nn.Module):
    ...
    def compute_mscn_coefficients_batch(self, images, kernel_size=7, sigma=7 / 6):
        ...
        kernel_1d = get_gaussian_kernel(kernel_size, sigma, device)

        kernel_2d = torch.outer(kernel_1d, kernel_1d)
        kernel_2d = kernel_2d.expand(C, 1, kernel_size, kernel_size)  # (C, 1, k, k)


        images = images.permute(0, 3, 1, 2)  # (B, C, H, W)


        mu = F.conv2d(images, kernel_2d, padding=kernel_size // 2, groups=C)
        mu_sq = mu ** 2


        sigma_sq = F.conv2d(images ** 2, kernel_2d, padding=kernel_size // 2, groups=C) - mu_sq
        sigma_map = torch.sqrt(torch.abs(sigma_sq)) + 1e-8


        mscn = (images - mu) / sigma_map


        local_sigma = sigma_map.permute(0, 2, 3, 1)
        return local_sigma

    def forward(self, x):
        B, H, W, C = x.shape
        org = x
        mscn = self.compute_mscn_coefficients_batch(x.to(torch.float32))

        x = torch.fft.rfft2(x, dim=(1, 2), norm='ortho')

        topk = x.shape[2]
        x_topk = torch.topk(mscn, topk, dim=2)[1]  # bs, nq
        x_topk = x_topk.to(torch.float32)

        weight = self.weight(x_topk.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
...
class PVT2FFN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.act(x)
        x = self.fc2(x)
        return x.view(B, H, W, C).transpose(1, 3).transpose(2, 3)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.attn(self.norm1(x.transpose(1, 3).transpose(1, 2))))  # x: b,c,h,w
        x = x + self.drop_path(self.mlp(self.norm2(x.transpose(1, 3).transpose(1, 2))))
        return x
