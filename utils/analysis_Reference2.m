function [EN, SF, SD, PSNR, MSE, MI, VIF, AG, CC, SCD, Qabf, DeltaE, Nabf, SSIM, MS_SSIM, FMI_pixel, FMI_dct, FMI_w] = analysis_Reference2(imgvi_rgb,imgf_rgb,img_f,img1,img2, easy)
    % 输入的img_f, img1, and img2 均是Uint8型，
    % 计算EN和MI是需要使用uint8的数据，其他指标均转换成float型的数据�?
    % 计算MSE和PSNR时数据归�?化到[0-1]，其他指标计算时数据的范围均是[0-255]
    grey_level = 256;
    [s1,s2] = size(img1);
    imgvi_rgb_float = im2double(imgvi_rgb);
    imgf_rgb_float = im2double(imgf_rgb);
    img1_int = img1;
    img2_int = img2;
    img_f_int = img_f;
    ...
    if easy ==1
        ...
        %SCD
        SCD = analysis_SCD(img1_float, img2_float, img_f_float);        
        %Qabf
        Qabf = analysis_Qabf(img1_float, img2_float, img_f_float);
        %deltaE
        DeltaE= deltaE2000(imgvi_rgb_float,imgf_rgb_float);
