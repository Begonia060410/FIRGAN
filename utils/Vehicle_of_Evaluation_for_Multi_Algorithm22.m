...
formatted_time = datestr(current_time, 'yyyy_mm_dd_HHMMSS');
...
for i = 1 : length(names)
    ...
    FMI_dct_set = []; FMI_w_set = [];
     for j = 1:num
        if (isequal(fileNames{j}, '.') || isequal(fileNames{j}, '..'))
            ...
            vi_image_rgb=vi_image;
            fused_image_rgb=fused_image;
        ...
        %     fused_image = fused_image(7:m-6, 7:n-6);
            ...
            if length(ir_size) < 3 && length(vi_size) < 3
                [EN, SF,SD,PSNR,MSE, MI, VIF, AG, CC, SCD, Qabf, DeltaE, Nabf, SSIM, MS_SSIM, FMI_pixel, FMI_dct, FMI_w] = analysis_Reference2(vi_image_rgb,fused_image_rgb,fused_image,ir_image,vi_image, easy);
                ...
                Qabf_set = [Qabf_set, Qabf]; DeltaE_set = [DeltaE_set, DeltaE];  Nabf_set = [Nabf_set, Nabf];
    ...
    file_name = fullfile(save_dir, strcat('Metric_',formatted_time,'_',dataset, '.xlsx')); %存放Excel文件的文件名
    ...
    if easy ==1
        ...
        writetable(DeltaE_table,file_name,'Sheet','DeltaE','Range',row_data);
        ...
        writetable(method_table,file_name,'Sheet','DeltaE','Range',row_name);
